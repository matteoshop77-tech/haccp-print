...
/* ── Label preview (mimics the physical 62mm label) ── */
function LabelPreview({ template, preparedDate }: {
  ...
  return (
    <div className="bg-white rounded-lg px-4 py-3 relative overflow-hidden">
      {/* Notch on right edge */}
      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-3 h-5
                      bg-app-bg rounded-l-full" />
      <p className="text-[9px] text-black/30 uppercase tracking-widest font-mono mb-1.5">
        HACCPrint
      </p>
      ...
      {template.description && (
        <p className="text-[10px] text-black/50 mt-1 leading-snug">
          {template.description}
        </p>
      )}
      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-[12px] font-semibold text-black">
            {formatLabelDate(preparedDate)}
          </p>
        </div>
        <div className="text-right">
          <p className="text-[9px] text-black/40 uppercase tracking-wide">Use by</p>
          <p className="text-[12px] font-semibold text-[#0F6E56]">
            {formatLabelDate(expiryDate)}
          </p>
        </div>
      </div>
      {template.allergens && (
        <p className="text-[9px] text-black/35 mt-2 leading-snug">
        ...
        <div className="flex items-start justify-between p-5 border-b border-app-border">
          <div>
            ...
            <p className="text-xs text-ink-muted mt-0.5">
              {template.category} · {t("home_last_print", lang)}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="nav-item w-7 h-7">
              <X size={15} />
            </button>
          </div>
        </div>
        ...
        <div className="px-5 pb-4 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-medium text-ink-primary w-8 text-center">
                {copies}
              </span>
              <button
            ...
            <span className="text-xs text-ink-faint">{t("print_max", lang)}</span>
          </div>

          {error && (
            <p className="text-xs text-coral bg-coral/10 border border-coral/20
